...
EXPERIMENT = 'compare_with_CN' # Experiment name
...
random_NT_lens = [6]*8 # Automatically determined in Jupyter Notebook pipeline.
...
if re.search(r'\/', DATA_SOURCE): # DATA_SOURCE = '../../../../tmp'
    assert Path(DATA_SOURCE).exists()
else:                            # DATA_SOURCE = 'intron.njms.rutgers.edu'
...
# Result directory
result_dir = project_dir/'results'/EXPERIMENT
...
if 'track_color' in sample_description.columns:
    sample_description.track_color = \
    sample_description.track_color.astype(int)
...
for folder_string in sample_description.folder_string.unique():
    ...
    if not re.search(r'\/', DATA_SOURCE):
        ...
        print(f'\nSearching folders with {folder_string} in their names on the server {DATA_SOURCE}')
        print(f'\nUsing command "{cmd}"')
        ...
        for file_string in df.file_string:
            ...
            for fastq_folder in fastq_folders:
                cmd = f'ssh {DATA_USER}@{DATA_SOURCE} find {fastq_folder} -type f -name "*{file_string}*R1*"'
                # print(cmd)
            ...
            cmd = f'scp -r {DATA_USER}@{DATA_SOURCE}:"{fastq_files}" {rawfastq_subdir}'
            print(cmd)
            os.system(cmd)
    ...
    # If fastq files are in a local folder:    
        ...
        rawfastq_subdir = check_output(cmd, shell=True).decode().strip() 
        print('\nLocal fastq folders found:', rawfastq_subdir)
    ...
    for file_string in df.file_string:
        ...
        # Get file names
        R1_fastq_gz = [str(filename) for filename in Path(rawfastq_subdir).iterdir() 
                       if re.search(file_string + '.*fastq.gz$', str(filename))]
        R1_fastq = [str(filename) for filename in Path(rawfastq_subdir).iterdir() 
        ...
        # Merge files
        ps.merge_and_rename(R1_fastq_gz, f'{sample_name}.fastq', rawfastq_dir)
        ps.merge_and_rename(R1_fastq, f'{sample_name}.fastq', rawfastq_dir)
    
    print('\nDone!')


# Determines number of workers for parallel computing:
...
# Count raw fastq records in fastq files
...
cmds = [(f'STAR --runThreadN {os.cpu_count()} --genomeDir {genome_index} --genomeLoad LoadAndKeep '
